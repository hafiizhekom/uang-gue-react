          ...
          <>
            ...
            <div className={`p-4 rounded-2xl border shadow-sm ${TREND_META[range.summary.overall_trend]?.color || TREND_META.stable.color}`}>
              ...
              {range.summary.avg_change_percent !== null && (
                <p className="text-[11px] font-bold opacity-70 mt-0.5">
                  Rata-rata {range.summary.avg_change_percent > 0 ? '+' : ''}{range.summary.avg_change_percent}% / period
                </p>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="p-3 rounded-2xl bg-rose-50 border border-rose-100">
                <p className="text-[8px] font-black uppercase tracking-widest text-rose-400">Paling Boros</p>
              ...
              <div className="p-3 rounded-2xl bg-emerald-50 border border-emerald-100">
                <p className="text-[8px] font-black uppercase tracking-widest text-emerald-500">Paling Hemat</p>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 space-y-3">
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Bulan ke Bulan</p>
                <h3 className="text-sm font-black text-slate-800 tracking-tight mt-0.5">Period Comparison</h3>
              </div>
              <div className="space-y-2">
                {range.periods.map((p) => (
                  <div key={p.id} className="flex items-center justify-between gap-2 bg-slate-50 rounded-xl p-3 border border-slate-100">
                    <div className="min-w-0">
                      <p className="text-xs font-black text-slate-800 truncate">{p.name}</p>
                      <p className="text-[10px] font-bold text-rose-500 mt-0.5">{formatShort(p.total_outcome)}</p>
                    </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-end justify-center bg-slate-900/60 backdrop-blur-sm" onClick={() => setPickerOpen(false)}>
          ...
          >
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h3 className="font-black text-slate-800 uppercase tracking-tighter text-lg">Pilih Rentang</h3>
